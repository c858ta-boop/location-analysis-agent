...
def make_color_excel(df, numeric_cols_list, type_col_name):
    """Генерация разукрашенного Excel на основе бизнес-логики доходов и расходов"""
    towrite = BytesIO()
    ...
    for r_idx in range(2, ws_export.max_row + 1):
        ...
        t_str = str(type_val_cell).strip().split('.')[0] # Убираем хвосты вроде .0
        is_income = "1" in t_str or "доход" in t_str
        ...
        for c_idx in range(1, ws_export.max_column + 1):
            ...
            if col_name in numeric_cols_list:
                ...
                if "(+" in cell_text:
                    ws_export.cell(row=r_idx, column=c_idx).fill = excel_fill_green if is_income else excel_fill_red
                elif "(-" in cell_text:
                    ws_export.cell(row=r_idx, column=c_idx).fill = excel_fill_red if is_income else excel_fill_green
    ...
    try:
        ...
        if sheet_target not in xl_1.sheet_names or sheet_target not in xl_2.sheet_names:
            ...
            if target_column not in df_1.columns or target_column not in df_2.columns:
                ...
                st.error(f"❌ Столбец типа '{type_column}' не найден в файле. Проверьте заголовки.")
            else:
                ...
                # Числовые столбцы (исключая Статью и Тип)
                ...
                for idx, row in df_result_raw.iterrows():
                    ...
                    raw_type_str = str(row[type_column]).strip().lower().split('.')[0]
                ...
                st.write("Индикация адаптирована: рост доходов и падение расходов подсвечены **зеленым**, падение доходов и рост расходов — **красным**.")
                st.dataframe(df_result.style.apply(style_cells, axis=None), use_container_width=True)
                
                st.write("---")
                st.subheader("📥 Экспорт результатов")
                st.write("Скачайте готовый отчет Excel. Цветовая бизнес-логика доходов/расходов будет полностью сохранена внутри файла.")
